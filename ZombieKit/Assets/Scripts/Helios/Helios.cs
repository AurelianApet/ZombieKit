...
public class Helios : MonoBehaviour {
	...
	public int 
		...
		selectedBuildingIndex = 0;		//necessary only for indestructible on/off before any units are deployed
	...
	public GameObject 
		...
		SmokePf, ExplosionPf, FirePf, RubbleBigPf, RubbleSmallPf, GravePf, GainGoldPf,  GainManaPf, //prefabs
	...
	*/
	...
	private int GetBuildingListIndex(int buildingIndex)//translates a real buildingIndex (ex: 32) into a possibly smaller list index (ex:2)
	{												   //this happends when the user builds and destroys buildings - the buildingIndex grows	
		int listIndex = 0;
		for (int i = 0; i < Buildings.Count; i++) 
		{
			if(Buildings[i].GetComponent<BuildingSelector>().buildingIndex == buildingIndex)
			{
				listIndex = i;
				break;
			}
	...
	{
		if (instantiationGroupIndex == -1||((Relay)relay).deploying) return;
	...
	private void SelectTargetGrid()
	{
		...
		for (int i = 0; i < aiTargetsChildren.Length; i++) // 1 skip parent
		{
			...
			if(i==0)
			{
				...
				{
				Talk (Buildings.Count.ToString() + " infected buildings, "+ Cannons.Count.ToString()+" defenses.");
				Talk ("Estimated value " +allLootGold.ToString() +" gold, "+ allLootMana.ToString() + " money.");
				}
			}
	...
	private void UpdatePaths()			//makes sure that not all units update paths at the same time - performance
	{
		if(updatePathO)
		{
			...
			if(updateOTimer > 0.2f)//time between nextunit updatePath
			{
				updateOTimer = 0;
		...
		else if(updatePathI)
		{
			...
			if(updateITimer > 0.2f)//time between nextunit updatePath
			{
				updateITimer = 0;
		...
		else if(updatePathII)
		{
			...
			if(updateIITimer > 0.2f)//time between nextunit updatePath
			{
				updateIITimer = 0;
		...
		else if(updatePathIII)
		{
			...
			if(updateIIITimer > 0.2f)//time between nextunit updatePath
			{
				updateIIITimer = 0;
	...
	void Update () 
	{
		...
		if (elapsedTime >= intervalTime) 
		{
			...
			int i = GetBuildingListIndex(buildingIndex);
			...
			{		
				...
				string buildingType = ((BuildingSelector)Buildings[i].GetComponent<BuildingSelector>()).buildingType;
				
				if(buildingType=="Chessboard"||buildingType=="Toolhouse"||buildingType=="Summon")
				{
					GameObject RubbleSmall = (GameObject)Instantiate (RubbleSmallPf, new Vector3 (Buildings [i].transform.position.x, Buildings [i].transform.position.y, rubbleZ), Quaternion.identity);
				}
				else
				{
					GameObject RubbleBig = (GameObject)Instantiate (RubbleBigPf, new Vector3 (Buildings [i].transform.position.x, Buildings [i].transform.position.y, rubbleZ), Quaternion.identity);
				}
				...
				{												
					...
					Buildings [i].transform.parent.FindChild ("Sprites").gameObject.SetActive (false);
				}

				for (int j = 0; j < targetBuildingIndex.Length; j++) //change the obstacles after a building is destroyed
				{
					if (targetBuildingIndex [j] == buildingIndex) 
					{
						...
						Transform AIObstaclesParentObj = grassTarget [j].transform.FindChild("AIObstacles");//AiObstacles parent object						
	...
	private void AllBuildingsIndestructible(bool b)
	{
		...
		for (int i = 0; i < BuildingsHealth.Count; i++) 
		{
			...
			int index = GetBuildingListIndex(i);

			CreateorDestroyStar(index,b);
		}
	}

	private bool ExistingBuildingStar(int i, bool b)//make sure the star doesn't already exist / destroy if required
	{
		...
		{
			if(((Selector)buildingStar.GetComponent("Selector")).index == i)		
			{
				if(b)				
					starExists = true;				
				else	
	...
	private int GetBuildingIndex(int listIndex)
	{
		int buildingIndex = Buildings[listIndex].GetComponent<BuildingSelector>().buildingIndex;
		return buildingIndex;
	}

	private void CreateBuildingStar(int index)
	{
		int i = GetBuildingIndex (index);
		...
		ProcessBuildingStar(index);
	}

	private void ProcessBuildingStar(int i)
	{
		GameObject[] buildingStarType = GameObject.FindGameObjectsWithTag("BuildingStar");
		foreach (GameObject buildingStar in buildingStarType) 
		{

			if(((Selector)buildingStar.GetComponent("Selector")).isSelected)		
			{
				buildingStar.transform.parent = EffectsGroup.transform;
				((Selector)buildingStar.GetComponent("Selector")).index = i;
				((Selector)buildingStar.GetComponent("Selector")).isSelected = false;
				break;
			}
		}
	}
	private void CreateorDestroyStar(int index, bool b)
	{
		if(b)
		{
			if(!ExistingBuildingStar(index, b))
			{
				CreateBuildingStar(index);
			}
		}
		else
			ExistingBuildingStar(index,b);
	...
	private void SelectedIndestructible(bool b)
	{
		...
		//and his targetBuildingIndex receives the Building list index of this building 
		//the buildingIndex may differ from Building list index - the user built and destroyed buildings
		//which makes it necessary to translate it like this: GetBuildingListIndex(targetBuildingIndex [k])
		
		int life = 100;
		if(b) life = 1000000;
		//to work independently of units deployed and targets
		int index = GetBuildingListIndex(selectedBuildingIndex);//targetBuildingIndex [selectedGroupIndex]

		CreateorDestroyStar(index,b);

		BuildingsHealth[index] = life;
	}
